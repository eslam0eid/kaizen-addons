...
class ProductProduct(models.Model):
    ...
    def _compute_totals_purchases(self):
        ...
        purchased_data = dict([(data[0], data[1]) for data in order_lines])
        for product in self:
            if not product.id:
                product.total_purchased = 0.0
                continue
            product.total_purchased = float_round(purchased_data.get(product, 0),
    ...
    def _compute_totals_sales(self):
        ...
        for group in self.env['sale.report']._read_group(domain, ['product_id', 'product_uom_qty']):
            r[group[0]] = group[1]
        for product in self:
            if not product.id:
                product.total_sales_product = 0.0
                continue
            product.total_sales_product = float_round(r.get(product, 0), precision_rounding=product.uom_id.rounding)
        return r
